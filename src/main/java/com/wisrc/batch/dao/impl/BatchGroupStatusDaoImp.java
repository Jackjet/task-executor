...
import com.wisrc.batch.dao.BatchGroupStatusDao;
...
public class BatchGroupStatusDaoImp implements BatchGroupStatusDao {
    ...
    public int init(BatchRunConfDto conf, Map<String, Integer> map) {
        ...
        // 删除当前批次的任务组历史信息
        jdbcTemplate.update(batchSqlText.getSql("sys_rdbms_172"), conf.getBatchId(), conf.getAsOfDate());
    ...
    public int setGroupRunning(BatchRunConfDto conf, String suiteKey, int status) {
        return jdbcTemplate.update(batchSqlText.getSql("sys_rdbms_188"), status, suiteKey, conf.getBatchId(), conf.getAsOfDate());
    }

    @Override
    public int setGroupEnd(BatchRunConfDto conf, String suiteKey, int status) {
        return jdbcTemplate.update(batchSqlText.getSql("sys_rdbms_189"), status, suiteKey, conf.getBatchId(), conf.getAsOfDate());
    }
}
